...
#define CLIENT_HEIGHT (1380)
#define CARD_WIDTH (224.22508f)
#define CARD_HEIGHT (312.80777f)
#define CARD_SCALE (1.0f)
#define CARD_OFFSET (60.0f)
#define BOARD_OFFSET (10.0f)
#define NOT_FOUND (-1)
#define ANIMATION_TIME (250)
#define MIN_CLIENT_WIDTH (480)
#define MIN_CLIENT_HEIGHT (346)

TCHAR szClassName[] = TEXT("FreeCell");

// The game is laid out on a fixed CLIENT_WIDTH x CLIENT_HEIGHT virtual canvas.
// When the window is resized, g_contentScale/g_contentOffset* describe how that
// virtual canvas maps onto the actual (resizable) client area: the content is
// scaled uniformly to fit and centered (letterboxed) so nothing is distorted.
float g_contentScale = 1.0f;
float g_contentOffsetX = 0.0f;
float g_contentOffsetY = 0.0f;
...
// Builds the transform that maps a point on the fixed virtual canvas
// (CLIENT_WIDTH x CLIENT_HEIGHT) to the actual, possibly resized, client area.
inline D2D1_MATRIX_3X2_F GetContentTransform()
{
	return D2D1::Matrix3x2F::Scale(g_contentScale, g_contentScale) * D2D1::Matrix3x2F::Translation(g_contentOffsetX, g_contentOffsetY);
}
...
public:
	...
	void Draw(ID2D1DeviceContext6* d2dDeviceContext, ID2D1Brush * brush) {
		...
		if (bSelected) {
			d2dDeviceContext->SetTransform(GetContentTransform());
			D2D1_RECT_F rect;
			rect.left = xx;
			rect.top = yy;
		...
		transform = transform * GetContentTransform();
...
public:
	...
	void AskNewGame() {
		if (MessageBox(hWnd, L"New Game？", L"New Game", MB_YESNOCANCEL) == IDYES) {
...
INT_PTR CALLBACK HelpDialogProc(HWND hDlg, unsigned msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		g = (Game*)lParam;
		CenterWindow(hDlg);
		SetDlgItemText(hDlg, IDC_EDIT_HELP, L"[How to play]\r\nFreecell is a card game that you can play alone according to the following rules."
			L"The purpose is to move all 52 cards to the 4 spaces called home cells on the upper right by using the 4 spaces called free cells on the upper left.\r\n"
			L"\r\n\r\n"
			L"Rule ①\r\nYou can move the top card of the row by dragging it with the mouse. However, the source card is only when the color (red or black) of the card in the top row of the destination is different and the number is one smaller. \r\n\r\n"
			L"Rule ②\r\nYou can freely place up to 4 cards in a free cell. In addition, cards placed in free cells can be moved to the top of the column when the conditions of rule 1 are met. \r\n\r\n"
			L"Rule ③\r\nIn the home cell, cards with 4 types of marks can be stacked from 1 to 13 in ascending order."
		);
		return TRUE;
	case WM_COMMAND:
		if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL)
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		int x = (int)((GET_X_LPARAM(lParam) - g_contentOffsetX) / g_contentScale);
		int y = (int)((GET_Y_LPARAM(lParam) - g_contentOffsetY) / g_contentScale);
	...
	case WM_GETMINMAXINFO:
	{
		...
		RECT rc = { 0, 0, MIN_CLIENT_WIDTH, MIN_CLIENT_HEIGHT };
		AdjustWindowRect(&rc, (DWORD)GetWindowLongPtr(hWnd, GWL_STYLE), TRUE);
	...
	case WM_SIZE:
	{
		...
		if (wParam != SIZE_MINIMIZED && width > 0 && height > 0 && dxgiSwapChain)
		{
			SafeRelease(bmpTarget);
			SafeRelease(dxgiBackBufferSurface);
			d2dDeviceContext->SetTarget(NULL);
			HRESULT hr = dxgiSwapChain->ResizeBuffers(0, width, height, DXGI_FORMAT_UNKNOWN, 0);
			if (SUCCEEDED(hr)) {
			...
			float scaleX = (float)width / CLIENT_WIDTH;
			float scaleY = (float)height / CLIENT_HEIGHT;
			g_contentScale = (scaleX < scaleY) ? scaleX : scaleY;
			g_contentOffsetX = ((float)width - CLIENT_WIDTH * g_contentScale) / 2.0f;
			g_contentOffsetY = ((float)height - CLIENT_HEIGHT * g_contentScale) / 2.0f;
